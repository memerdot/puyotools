...
namespace puyo_tools
{
    public class LZ01_CXLZ_Decompress : Form
    {
        ...
        public LZ01_CXLZ_Decompress()
        {
            /* Select our files */
            selectFiles                  = new OpenFileDialog();
            selectFiles.Title            = "Select LZ01 / CXLZ Compressed file(s)";
            ...
            selectFiles.Filter           = "LZ01 / CXLZ Compressed Files|*.*";
            ...
            this.Text            = "LZ01 / CXLZ Decompressor Options";
        ...
        private void startWork(object sender, EventArgs e)
        {
            ...
            status = new StatusMessage("Decompress LZ01 / CXLZ Compressed File", StatusMessage.decompress, selectFiles.FileNames.Length);
            status.Show();
            ...
            for (int i = 0; i < selectFiles.FileNames.Length; i++)
            {
                try
                {
                    ...
                    if (Header.isFile(data, Header.LZ01, 0))
                    {
                        ...
                        outputDir = Path.GetDirectoryName(selectFiles.FileNames[i]) + Path.DirectorySeparatorChar + ExtractDir.LZ01;
                    }
                    ...
                    else if (Header.isFile(data, Header.CXLZ, 0))
                    {
                        ...
                        /* Set directory for output files */
                        outputDir = Path.GetDirectoryName(selectFiles.FileNames[i]) + Path.DirectorySeparatorChar + ExtractDir.CXLZ;
                    }
