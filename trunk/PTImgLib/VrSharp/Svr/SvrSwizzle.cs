        ...
        public static void UnSwizzle4(byte[] Buf, int Width, int Height, int Where)
        {
            ...
            for (int y = 0; y < Height; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    ...
                    byte uPen = (byte)(Swizzled[page_location + block_location + column_location + byte_num]);
                    Buf[Where + (index >> 1)] = (byte)((Buf[Where + (index >> 1)] & (0xF0 >> ((index % 2) * 4))) | (uPen & (0xF << (bits_set * 4))));
                }
            }
        }
